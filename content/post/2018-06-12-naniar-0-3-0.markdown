...
I've written in more detail about how to get started with missing data in this ["getting started"](http://naniar.njtierney.com/articles/getting-started-w-naniar.html). I would recommend starting there for a quick tour of the features of `naniar`.
...
There are also `is_shadow` and `are_shadow` functions, to determine if something contains a shadow column. Similar to `rlang::is_na` and `rland::are_na`, `is_shadow` This might be revisited at a later point to reflect the "shadow values" as "shades". See `is_shade` and `any_shade` in `add_label_shadow`.
